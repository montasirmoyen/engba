...
#ifdef __cplusplus
#define VECTOR_CAST(type, expr) static_cast<type>(expr)
#else
#define VECTOR_CAST(type, expr) (expr)
#endif
...
#define DEFINE_VECTOR(NAME, TYPE) \
	void NAME ## Init(struct NAME* vector, size_t capacity) { \
		...
		vector->vector = VECTOR_CAST(TYPE*, calloc(capacity, sizeof(TYPE))); \
	} \
	void NAME ## Deinit(struct NAME* vector) { \
		free(vector->vector); \
		vector->vector = 0; \
	...
	void NAME ## EnsureCapacity(struct NAME* vector, size_t capacity) { \
		...
		vector->vector = VECTOR_CAST(TYPE*, realloc(vector->vector, vector->capacity * sizeof(TYPE))); \
	} \
	void NAME ## Shift(struct NAME* vector, size_t location, size_t difference) { \
		VECTOR_BOUNDS_CHECK(NAME, vector, location); \
