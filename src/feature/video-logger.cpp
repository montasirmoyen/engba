...
void mVideoLoggerRendererInit(struct mVideoLogger* logger) {
	logger->palette = static_cast<uint16_t*>(anonymousMemoryMap(logger->paletteSize));
	logger->vram = static_cast<uint16_t*>(anonymousMemoryMap(logger->vramSize));
	logger->oam = static_cast<uint16_t*>(anonymousMemoryMap(logger->oamSize));

	logger->vramDirtyBitmap = static_cast<uint32_t*>(calloc(_roundUp(logger->vramSize, 17), sizeof(uint32_t)));
	logger->oamDirtyBitmap = static_cast<uint32_t*>(calloc(_roundUp(logger->oamSize, 6), sizeof(uint32_t)));
...
bool mVideoLoggerRendererRun(struct mVideoLogger* logger, bool block) {
	struct mVideoLogChannel* channel = static_cast<struct mVideoLogChannel*>(logger->dataContext);
	...
	struct mVideoLoggerDirtyInfo buffer = {DIRTY_DUMMY, 0, 0, 0};
	struct mVideoLoggerDirtyInfo item = {DIRTY_DUMMY, 0, 0, 0};
	while (logger->readData(logger, &buffer, sizeof(buffer), block)) {
		uint32_t tempType;
		LOAD_32LE(tempType, 0, &buffer.type);
		item.type = static_cast<enum mVideoLoggerDirtyType>(tempType);
...
struct mVideoLogContext* mVideoLogContextCreate(struct mCore* core) {
	struct mVideoLogContext* context = static_cast<struct mVideoLogContext*>(malloc(sizeof(*context)));
...
static const struct mVLDescriptor* _mVideoLogDescriptor(struct VFile* vf) {
	...
	enum mPlatform platform;
	uint32_t tempPlatform;
	LOAD_32LE(tempPlatform, 0, &header.platform);
	platform = static_cast<enum mPlatform>(tempPlatform);
