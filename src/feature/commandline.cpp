...
static void _tableApply(const char* key, void* value, void* user) {
	struct mCoreConfig* config = static_cast<struct mCoreConfig*>(user);
	mCoreConfigSetOverrideValue(config, key, static_cast<const char*>(value));
}
...
bool _parseGraphicsArg(struct mSubParser* parser, int option, const char* arg) {
	UNUSED(arg);
	struct mGraphicsOpts* graphicsOpts = static_cast<struct mGraphicsOpts*>(parser->opts);
	switch (option) {
	case 'f':
