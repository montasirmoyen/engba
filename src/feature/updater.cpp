...
static void _updateListSections(const char* sectionName, void* user) {
	struct StringList* out = static_cast<struct StringList*>(user);
...
static void _updateList(const char* key, const char* value, void* user) {
	...
	struct Table* out = static_cast<struct Table*>(user);
	struct mUpdate* update = static_cast<struct mUpdate*>(HashTableLookup(out, channel));
	if (!update) {
		update = static_cast<struct mUpdate*>(calloc(1, sizeof(*update)));
...
static void _updateMatch(const char* key, const char* value, void* user) {
	struct mUpdateMatch* match = static_cast<struct mUpdateMatch*>(user);
