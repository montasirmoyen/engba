...
static struct mInputMapImpl* _guaranteeMap(struct mInputMap* map, uint32_t type) {
	struct mInputMapImpl* impl = 0;
	if (map->numMaps == 0) {
		map->maps = static_cast<struct mInputMapImpl*>(malloc(sizeof(*map->maps)));
		map->numMaps = 1;
		impl = &map->maps[0];
		impl->type = type;
		impl->map = static_cast<int*>(calloc(map->info->nKeys, sizeof(int)));
		size_t i;
	...
	if (!impl) {
		...
		if (impl) {
			...
			map->maps = static_cast<struct mInputMapImpl*>(realloc(map->maps, sizeof(*map->maps) * map->numMaps * 2));
...
static void _saveAxis(uint32_t axis, void* dp, void* up) {
	struct mInputAxisSave* user = static_cast<struct mInputAxisSave*>(up);
	const struct mInputAxis* description = static_cast<const struct mInputAxis*>(dp);
...
void _enumerateAxis(uint32_t axis, void* dp, void* ep) {
	struct mInputAxisEnumerate* enumUser = static_cast<struct mInputAxisEnumerate*>(ep);
	const struct mInputAxis* description = static_cast<const struct mInputAxis*>(dp);
...
void _unbindAxis(uint32_t axis, void* dp, void* user) {
	UNUSED(axis);
	int* key = static_cast<int*>(user);
	struct mInputAxis* description = static_cast<struct mInputAxis*>(dp);
...
void _enumerateHat(uint32_t axis, void* dp, void* ep) {
	struct mInputHatEnumerate* enumUser = static_cast<struct mInputHatEnumerate*>(ep);
	const struct mInputHatBindings* description = static_cast<const struct mInputHatBindings*>(dp);
	enumUser->handler(axis, description, enumUser->user);
}
...
int mInputMapAxis(const struct mInputMap* map, uint32_t type, int axis, int16_t value) {
	...
	struct mInputAxis* description = static_cast<struct mInputAxis*>(TableLookup(&impl->axes, axis));
	if (!description) {
		return -1;
	}
	int state = 0;
...
void mInputBindAxis(struct mInputMap* map, uint32_t type, int axis, const struct mInputAxis* description) {
	...
	struct mInputAxis* dup = static_cast<struct mInputAxis*>(malloc(sizeof(struct mInputAxis)));
	*dup = *description;
...
const struct mInputAxis* mInputQueryAxis(const struct mInputMap* map, uint32_t type, int axis) {
	...
	return static_cast<const struct mInputAxis*>(TableLookup(&impl->axes, axis));
}
