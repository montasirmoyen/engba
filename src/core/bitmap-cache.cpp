...
static void _redoCacheSize(struct mBitmapCache* cache) {
	...
	size_t size = mBitmapCacheSystemInfoGetHeight(cache->sysConfig) * mBitmapCacheSystemInfoGetBuffers(cache->sysConfig);
	cache->cache = static_cast<mColor*>(anonymousMemoryMap(mBitmapCacheSystemInfoGetWidth(cache->sysConfig) * size * sizeof(mColor)));
	cache->status = static_cast<struct mBitmapCacheEntry*>(anonymousMemoryMap(size * sizeof(*cache->status)));
	if (mBitmapCacheSystemInfoIsUsesPalette(cache->sysConfig)) {
		cache->palette = static_cast<mColor*>(calloc((1 << (1 << mBitmapCacheSystemInfoGetEntryBPP(cache->sysConfig))), sizeof(mColor)));
	} else {
		cache->palette = NULL;
	}
}
