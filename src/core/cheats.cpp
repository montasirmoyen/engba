...
static void _patchROM(struct mCheatDevice* device, struct mCheatSet* cheats) {
	...
	for (i = 0; i < mCheatPatchListSize(&cheats->romPatches); ++i) {
		...
		struct mCheatPatchedMem* patchData = static_cast<struct mCheatPatchedMem*>(TableLookup(&device->unpatchedMemory, patchKey));
		if (!patchData) {
			patchData = static_cast<struct mCheatPatchedMem*>(malloc(sizeof(*patchData)));
