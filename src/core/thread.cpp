...
void _frameStarted(void* context) {
	struct mCoreThread* thread = static_cast<struct mCoreThread*>(context);
	if (!thread) {
		return;
	}
...
static THREAD_ENTRY _mCoreThreadRun(void* context) {
	struct mCoreThread* threadContext = static_cast<struct mCoreThread*>(context);
#ifdef USE_PTHREADS
...
bool mCoreThreadStart(struct mCoreThread* threadContext) {
	threadContext->impl = static_cast<struct mCoreThreadInternal*>(calloc(1, sizeof(*threadContext->impl)));
...
struct mCoreThread* mCoreThreadGet(void) {
	...
	InitOnceExecuteOnce(&_contextOnce, _createTLS, NULL, 0);
#endif
	return static_cast<struct mCoreThread*>(ThreadLocalGetValue(_contextKey));
}
