...
static void _redoCacheSize(struct mMapCache* cache) {
	...
	cache->cache = static_cast<mColor*>(anonymousMemoryMap(8 * 8 * sizeof(mColor) * tiles));
	cache->status = static_cast<struct mMapCacheEntry*>(anonymousMemoryMap(tiles * sizeof(*cache->status)));
}
