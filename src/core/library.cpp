...
static void _bindConstraints(sqlite3_stmt* statement, const struct mLibraryEntry* constraints) {
	...
	static const uint8_t zeroMd5[16] = {0};
	if (memcmp(constraints->md5, zeroMd5, 16) != 0) {
	...
	static const uint8_t zeroSha1[20] = {0};
	if (memcmp(constraints->sha1, zeroSha1, 20) != 0) {
...
struct mLibrary* mLibraryLoad(const char* path) {
	struct mLibrary* library = static_cast<struct mLibrary*>(malloc(sizeof(*library)));
	memset(library, 0, sizeof(*library));

	int romsTableVersion; // Declared early to avoid goto jumping over initialization
	...
	romsTableVersion = _mLibraryTableVersion(library, "roms");
	...
	for (entryIndex = 0; (!numEntries || entryIndex < numEntries) && sqlite3_step(library->select) == SQLITE_ROW; ++entryIndex) {
		...
		for (i = 0; i < nCols; ++i) {
			...
			if (strcmp(colName, "sha1") == 0) {
				...
				entry->platform = static_cast<enum mPlatform>(sqlite3_column_int(library->select, i));
