...
#ifndef DISABLE_THREADING
	_setupTLS();
	return static_cast<struct mLogger*>(ThreadLocalGetValue(_contextKey));
#else
	return NULL;
#endif
}
...
static void _setFilterLevel(const char* key, const char* value, enum mCoreConfigLevel level, void* user) {
	UNUSED(level);
	struct mLogFilter* filter = static_cast<struct mLogFilter*>(user);
	key = strchr(key, '.');
	if (!key || !key[1]) {
		return;
	}
...
void mStandardLoggerInit(struct mStandardLogger* logger) {
	...
	logger->d.filter = static_cast<struct mLogFilter*>(malloc(sizeof(struct mLogFilter)));
