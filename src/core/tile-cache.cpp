...
static void _redoCacheSize(struct mTileCache* cache) {
	...
	unsigned tiles = mTileCacheSystemInfoGetMaxTiles(cache->sysConfig);
	cache->cache = static_cast<mColor*>(anonymousMemoryMap(8 * 8 * sizeof(mColor) * tiles * size));
	cache->status = static_cast<struct mTileCacheEntry*>(anonymousMemoryMap(tiles * size * sizeof(*cache->status)));
	cache->globalPaletteVersion = static_cast<uint32_t*>(calloc(size, sizeof(*cache->globalPaletteVersion)));
	cache->palette = static_cast<mColor*>(calloc(size * bpp, sizeof(*cache->palette)));
}
...
const mColor* mTileCacheGetTile(struct mTileCache* cache, unsigned tileId, unsigned paletteId) {
	...
	struct mTileCacheEntry desiredStatus = {
		...
		.vramClean = 1,
		.paletteId = static_cast<uint8_t>(paletteId)
	};
