...
bool mStateExtdataSerialize(struct mStateExtdata* extdata, struct VFile* vf) {
	...
	struct mStateExtdataHeader* header = static_cast<struct mStateExtdataHeader*>(malloc(size));
	position += size;
...
bool mStateExtdataDeserialize(struct mStateExtdata* extdata, struct VFile* vf) {
	while (true) {
		...
		mStateExtdataPut(extdata, static_cast<enum mStateExtdataTag>(header.tag), &item);
...
static bool _savePNGState(struct mCore* core, struct VFile* vf, struct mStateExtdata* extdata, const void* pixels, size_t stride) {
	...
	compress(static_cast<Bytef*>(buffer), &len, (const Bytef*) state, stateSize);
	mappedMemoryFree(state, stateSize);
	...
	if (extdata) {
		...
		for (i = 1; i < EXTDATA_MAX; ++i) {
			...
			uint32_t* data = static_cast<uint32_t*>(malloc(len));
			if (!data) {
				continue;
			}
			STORE_32LE(i, 0, data);
			STORE_32LE(extdata->data[i].size, sizeof(uint32_t), data);
			compress((Bytef*) (data + 2), &len, static_cast<const Bytef*>(extdata->data[i].data), extdata->data[i].size);
...
static int _loadPNGChunkHandler(png_structp png, png_unknown_chunkp chunk) {
	struct mBundledState* bundle = static_cast<struct mBundledState*>(png_get_user_chunk_ptr(png));
	if (!bundle) {
		return 0;
	}
	...
	if (!strcmp((const char*) chunk->name, "gbAx")) {
		...
		if (uncompress((Bytef*) item.data, &len, data, chunk->size) == Z_OK) {
			...
			mStateExtdataPut(extdata, static_cast<enum mStateExtdataTag>(tag), &item);
		} else {
...
static void* _loadPNGState(struct mCore* core, struct VFile* vf, struct mStateExtdata* extdata) {
	...
	uint32_t* pixels = static_cast<uint32_t*>(malloc(width * height * 4));
	if (!pixels) {
		PNGReadClose(png, info, end);
		return NULL;
	}
	...
	if (!success) {
		...
		struct mStateExtdataItem item = {
			.size = static_cast<int32_t>(width * height * 4),
			.data = pixels,
			.clean = free
		};
		mStateExtdataPut(extdata, EXTDATA_SCREENSHOT, &item);

		uint16_t dims[2] = { static_cast<uint16_t>(width), static_cast<uint16_t>(height) };
...
static bool _loadPNGExtdata(struct VFile* vf, struct mStateExtdata* extdata) {
	...
	pixels = static_cast<uint32_t*>(malloc(width * height * 4));
	if (!pixels) {
		PNGReadClose(png, info, end);
		return false;
	}
...
bool mCoreSaveStateNamed(struct mCore* core, struct VFile* vf, int flags) {
	...
	if (flags & SAVESTATE_METADATA) {
		uint64_t* creationUsec = static_cast<uint64_t*>(malloc(sizeof(*creationUsec)));
		if (creationUsec) {
#ifndef _MSC_VER
			struct timeval tv;
		...
		struct mStateExtdataItem item = {
			.size = static_cast<int32_t>(strlen(creator) + 1),
			.data = strdup(creator),
			.clean = free
		};
	...
	if (flags & SAVESTATE_CHEATS && (device = core->cheatDevice(core))) {
		...
		if (cheatVf) {
			...
			struct mStateExtdataItem item = {
				.size = static_cast<int32_t>(cheatVf->size(cheatVf)),
