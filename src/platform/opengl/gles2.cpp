...
static void mGLES2ContextInit(struct VideoBackend* v, WHandle handle) {
	...
	struct mGLES2Uniform* uniforms = static_cast<struct mGLES2Uniform*>(malloc(sizeof(struct mGLES2Uniform) * 4));
...
static void _uniformHandler(const char* sectionName, void* user) {
	struct mGLES2UniformList* uniforms = static_cast<struct mGLES2UniformList*>(user);
	unsigned passId;
	int sentinel;
...
bool mGLES2ShaderLoad(struct VideoShader* shader, struct VDir* dir) {
	...
	if (ConfigurationReadVFile(&description, manifest)) {
		...
		if (success) {
			struct mGLES2Shader* shaderBlock = static_cast<struct mGLES2Shader*>(calloc(inShaders, sizeof(struct mGLES2Shader)));
			int n;
			for (n = 0; n < inShaders; ++n) {
				...
				if (fs) {
					...
					fssrc = static_cast<char*>(malloc(fsf->size(fsf) + 1));
				...
				if (vs) {
					...
					vssrc = static_cast<char*>(malloc(vsf->size(vsf) + 1));
				...
				if (u) {
					uniformBlock = static_cast<struct mGLES2Uniform*>(calloc(u, sizeof(*uniformBlock)));
...
void mGLES2ShaderFree(struct VideoShader* shader) {
	...
	struct mGLES2Shader* shaders = static_cast<struct mGLES2Shader*>(shader->passes);
	size_t n;
