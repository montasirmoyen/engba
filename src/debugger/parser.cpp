...
static void _lexOperator(struct LexVector* lv, char op, enum LexState* state) {
	if (*state == LEX_EXPECT_OPERATOR2) {
		struct Token* lvNext = LexVectorGetPointer(lv, LexVectorSize(lv) - 1);
		if (lvNext->type != Token::TOKEN_OPERATOR_TYPE) {
			lvNext->type = Token::TOKEN_ERROR_TYPE;
			*state = LEX_ERROR;
			return;
		}
	...
	struct Token lvNext;
	lvNext.type = Token::TOKEN_OPERATOR_TYPE;
	switch (op) {
	case '=':
...
static void _lexValue(struct LexVector* lv, char token, uint32_t next, enum LexState* state) {
	...
	case '!':
		...
		lvNext->type = Token::TOKEN_UINT_TYPE;
	...
	case ')':
		...
		lvNext->uintValue = next;
		lvNext = LexVectorAppend(lv);
		lvNext->type = Token::TOKEN_CLOSE_PAREN_TYPE;
		*state = LEX_ROOT;
		break;
	case ' ':
	case '\t':
	...
	while (length > 0 && string[0] && !strchr(eol, string[0]) && state != LEX_ERROR) {
		...
		case LEX_ROOT:
			...
			case '(':
				state = LEX_ROOT;
				lvNext = LexVectorAppend(lv);
				lvNext->type = Token::TOKEN_OPEN_PAREN_TYPE;
				break;
			case '=':
			case '+':
			case '-':
		...
		case LEX_EXPECT_IDENTIFIER:
			...
			case '~':
				lvNext = LexVectorAppend(lv);
				lvNext->type = Token::TOKEN_IDENTIFIER_TYPE;
		...
		case LEX_EXPECT_HEX:
			...
			case ':':
				lvNext = LexVectorAppend(lv);
				lvNext->type = Token::TOKEN_SEGMENT_TYPE;
				lvNext->uintValue = next;
				next = 0;
				break;
			default:
...
struct ParseTree* parseTreeCreate(void) {
	struct ParseTree* tree = static_cast<struct ParseTree*>(malloc(sizeof(struct ParseTree)));
	tree->token.type = Token::TOKEN_ERROR_TYPE;
	tree->p = NULL;
	tree->rhs = NULL;
	tree->lhs = NULL;
...
static size_t _parseExpression(struct ParseTree* tree, struct LexVector* lv, int* openParens) {
	...
	while (i < LexVectorSize(lv)) {
		...
		int newPrecedence;
		switch (token->type) {
		case Token::TOKEN_IDENTIFIER_TYPE:
		case Token::TOKEN_UINT_TYPE:
			if (tree->token.type == Token::TOKEN_ERROR_TYPE) {
				tree->token = *token;
				if (token->type == Token::TOKEN_IDENTIFIER_TYPE) {
			...
			break;
		case Token::TOKEN_SEGMENT_TYPE:
			tree->lhs = parseTreeCreate();
			tree->lhs->token.type = Token::TOKEN_UINT_TYPE;
			...
			tree->token.type = Token::TOKEN_SEGMENT_TYPE;
			tree = tree->rhs;
			++i;
			break;
		case Token::TOKEN_OPEN_PAREN_TYPE:
			++*openParens;
			precedence = INT_MAX;
			++i;
			break;
		case Token::TOKEN_CLOSE_PAREN_TYPE:
			if (*openParens <= 0) {
				tree->token.type = Token::TOKEN_ERROR_TYPE;
			}
			--*openParens;
			...
			pop = true;
			break;
		case Token::TOKEN_OPERATOR_TYPE:
			if (tree->token.type == Token::TOKEN_ERROR_TYPE) {
		...
		if (pop) {
			if (tree->token.type == Token::TOKEN_ERROR_TYPE && tree->p) {
				tree->p->token.type = Token::TOKEN_ERROR_TYPE;
			}
			tree = tree->p;
			pop = false;
			if (!tree) {
...
bool parseLexedExpression(struct ParseTree* tree, struct LexVector* lv) {
	...
	if (openParens) {
		if (tree->token.type == Token::TOKEN_IDENTIFIER_TYPE) {
			free(tree->token.identifierValue);
		}
		tree->token.type = Token::TOKEN_ERROR_TYPE;
	}
	return tree->token.type != Token::TOKEN_ERROR_TYPE;
}
