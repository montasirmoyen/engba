...
static bool _parseExpression(struct mDebuggerModule* debugger, struct CLIDebugVector* dv, int32_t* intValue, int* segmentValue) {
	...
	const char** arglist = static_cast<const char**>(calloc(args + 1, sizeof(const char*)));
	args = 0;
	for (accum = dv; accum; accum = accum->next) {
...
static void _readByte(struct CLIDebugger* debugger, struct CLIDebugVector* dv) {
	if (!dv || dv->type != CLIDebugVector::CLIDV_INT_TYPE) {
		debugger->backend->printf(debugger->backend, "%s\n", ERROR_MISSING_ARGS);
		return;
	}
...
static void _writeByte(struct CLIDebugger* debugger, struct CLIDebugVector* dv) {
	...
	if (dv->type != CLIDebugVector::CLIDV_INT_TYPE || dv->next->type != CLIDebugVector::CLIDV_INT_TYPE) {
		debugger->backend->printf(debugger->backend, "%s\n", ERROR_INVALID_ARGS);
		return;
	}
	uint32_t address = dv->intValue;
...
static void _writeRegister(struct CLIDebugger* debugger, struct CLIDebugVector* dv) {
	...
	if (dv->type != CLIDebugVector::CLIDV_CHAR_TYPE || dv->next->type != CLIDebugVector::CLIDV_INT_TYPE) {
		debugger->backend->printf(debugger->backend, "%s\n", ERROR_INVALID_ARGS);
		return;
	}
...
static void _dumpByte(struct CLIDebugger* debugger, struct CLIDebugVector* dv) {
	...
	uint32_t address = dv->intValue;
	uint32_t words = 16;
	if (dv->next && dv->next->type == CLIDebugVector::CLIDV_INT_TYPE) {
...
static void _setBreakpoint(struct CLIDebugger* debugger, struct CLIDebugVector* dv) {
	...
	struct mBreakpoint breakpoint = {
		.address = static_cast<uint32_t>(dv->intValue),
		.segment = dv->segmentValue,
		.type = BREAKPOINT_HARDWARE,
	};
	if (dv->next && dv->next->type == CLIDebugVector::CLIDV_CHAR_TYPE) {
...
static void _setWatchpoint(struct CLIDebugger* debugger, struct CLIDebugVector* dv, enum mWatchpointType type) {
	...
	struct mWatchpoint watchpoint = {
		.segment = dv->segmentValue,
		.minAddress = static_cast<uint32_t>(dv->intValue),
		.maxAddress = static_cast<uint32_t>(dv->intValue + 1),
		.type = type,
	};
	if (dv->next && dv->next->type == CLIDebugVector::CLIDV_CHAR_TYPE) {
		struct ParseTree* tree = _parseTree((const char*[]) { dv->next->charValue, NULL });
...
static void _setRangeWatchpoint(struct CLIDebugger* debugger, struct CLIDebugVector* dv, enum mWatchpointType type) {
	...
	struct mWatchpoint watchpoint = {
		.segment = dv->segmentValue,
		.minAddress = static_cast<uint32_t>(dv->intValue),
		.maxAddress = static_cast<uint32_t>(dv->next->intValue),
		.type = type,
	};
	if (dv->next->next && dv->next->next->type == CLIDebugVector::CLIDV_CHAR_TYPE) {
...
static void _enableBreakpoint(struct CLIDebugger* debugger, struct CLIDebugVector* dv) {
	...
	while (current) {
		if (current->type == CLIDebugVector::CLIDV_INT_TYPE) {
...
static void _trace(struct CLIDebugger* debugger, struct CLIDebugVector* dv) {
	...
	if (dv->type != CLIDebugVector::CLIDV_INT_TYPE || dv->intValue < 0) {
		debugger->backend->printf(debugger->backend, "%s\n", ERROR_INVALID_ARGS);
		return;
	}
...
struct CLIDebugVector* CLIDVParse(struct CLIDebugger* debugger, const char* string, size_t length) {
	...
	struct CLIDebugVector dvTemp = { .type = CLIDebugVector::CLIDV_INT_TYPE, .segmentValue = -1 };

	struct LexVector lv;
	LexVectorInit(&lv, 0);
	size_t adjusted = lexExpression(&lv, string, length, " ");
	if (adjusted > length) {
		dvTemp.type = CLIDebugVector::CLIDV_ERROR_TYPE;
	}
	...
	struct CLIDebugVector* dv = static_cast<struct CLIDebugVector*>(malloc(sizeof(struct CLIDebugVector)));
	if (dvTemp.type == CLIDebugVector::CLIDV_ERROR_TYPE) {
		dv->type = CLIDebugVector::CLIDV_ERROR_TYPE;
		dv->next = 0;
	} else {
		*dv = dvTemp;
	}
...
struct CLIDebugVector* CLIDVStringParse(struct CLIDebugger* debugger, const char* string, size_t length) {
	...
	struct CLIDebugVector dvTemp = { .type = CLIDebugVector::CLIDV_CHAR_TYPE };
...
static void _DVFree(struct CLIDebugVector* dv) {
	...
	while (dv) {
		next = dv->next;
		if (dv->type == CLIDebugVector::CLIDV_CHAR_TYPE) {
			free(dv->charValue);
		}
		free(dv);
		dv = next;
		...
		if (strncasecmp(name, command, commandLen) == 0) {
			if (commands[i].format && args) {
				...
				for (arg = 0; commands[i].format[arg] && argsLen; ++arg) {
					...
					if (dvNext->type == CLIDebugVector::CLIDV_ERROR_TYPE) {
						debugger->backend->printf(debugger->backend, "Parse error\n");
						_DVFree(dv);
						_DVFree(dvNext);
						return 0;
...
static void _backtrace(struct CLIDebugger* debugger, struct CLIDebugVector* dv) {
	...
	if (dv && dv->type == CLIDebugVector::CLIDV_INT_TYPE && dv->intValue < frames) {
		frames = dv->intValue;
	}
	ssize_t i;
...
static void _findSymbol(struct CLIDebugger* debugger, struct CLIDebugVector* dv) {
	...
	if (dv->type != CLIDebugVector::CLIDV_INT_TYPE) {
		debugger->backend->printf(debugger->backend, "%s\n", ERROR_INVALID_ARGS);
		return;
	}
