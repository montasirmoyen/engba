...
void GBAMemoryInit(struct GBA* gba) {
	...
	gba->memory.wram = static_cast<uint32_t*>(anonymousMemoryMap(GBA_SIZE_EWRAM + GBA_SIZE_IWRAM));
		...
		if ((address & 0x00FFFFFF) >= AGB_PRINT_BASE) {
			...
			if (agbPrintAddr == AGB_PRINT_PROTECT) {
				...
				if (!memory->agbPrintBuffer) {
					memory->agbPrintBuffer = static_cast<uint16_t*>(anonymousMemoryMap(GBA_SIZE_AGB_PRINT));
					if (memory->romSize >= GBA_SIZE_ROM0 / 2) {
						...
						memory->agbPrintBufferBackup = static_cast<uint16_t*>(anonymousMemoryMap(GBA_SIZE_AGB_PRINT));
