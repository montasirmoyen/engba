...
static void _addBreakpoint(struct mCheatDevice* device, struct GBACheatSet* cheats) {
	...
	GBASetBreakpoint(static_cast<struct GBA*>(device->p->board), &device->d, cheats->hook->address, cheats->hook->mode, &cheats->hook->patchedOpcode);
}
...
static void _removeBreakpoint(struct mCheatDevice* device, struct GBACheatSet* cheats) {
	...
	GBAClearBreakpoint(static_cast<struct GBA*>(device->p->board), cheats->hook->address, cheats->hook->mode, cheats->hook->patchedOpcode);
}
...
static struct mCheatSet* GBACheatSetCreate(struct mCheatDevice* device, const char* name) {
	UNUSED(device);
	struct GBACheatSet* set = static_cast<struct GBACheatSet*>(malloc(sizeof(*set)));
...
struct mCheatDevice* GBACheatDeviceCreate(void) {
	struct mCheatDevice* device = static_cast<struct mCheatDevice*>(malloc(sizeof(*device)));
