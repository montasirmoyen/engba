...
static void GBAInit(void* cpu, struct mCPUComponent* component) {
	...
	gba->cpu = static_cast<struct ARMCore*>(cpu);
	gba->debugger = 0;
	gba->sync = 0;
...
bool GBALoadNull(struct GBA* gba) {
	...
	gba->pristineRomSize = 0;
#ifndef FIXED_ROM_BUFFER
	gba->memory.rom = static_cast<uint32_t*>(anonymousMemoryMap(GBA_SIZE_ROM0));
#else
...
bool GBALoadROM(struct GBA* gba, struct VFile* vf) {
	...
	if (gba->pristineRomSize > GBA_SIZE_ROM0) {
		...
		if (ident == 'M') {
			...
			gba->memory.rom = static_cast<uint32_t*>(vf->map(vf, GBA_SIZE_ROM0, MAP_READ));
			gba->memory.romSize = GBA_SIZE_ROM0;
		}
		...
		gba->memory.rom = static_cast<uint32_t*>(vf->map(vf, gba->pristineRomSize, MAP_READ));
...
#ifndef FIXED_ROM_BUFFER
		uint32_t* newRom = static_cast<uint32_t*>(anonymousMemoryMap(GBA_SIZE_ROM0));
...
void GBALoadBIOS(struct GBA* gba, struct VFile* vf) {
	...
	uint32_t* bios = static_cast<uint32_t*>(vf->map(vf, GBA_SIZE_BIOS, MAP_READ));
	if (!bios) {
...
void GBADebug(struct GBA* gba, uint16_t flags) {
	...
	if (GBADebugFlagsIsSend(gba->debugFlags)) {
		...
		memset(gba->debugString, 0, sizeof(gba->debugString));
		oolBuf[0x100] = '\0';
		mLog(_mLOG_CAT_GBA_DEBUG, static_cast<enum mLogLevel>(level), "%s", oolBuf);
	}
...
static void _triggerIRQ(struct mTiming* timing, void* user, uint32_t cyclesLate) {
	UNUSED(timing);
	UNUSED(cyclesLate);
	struct GBA* gba = static_cast<struct GBA*>(user);
	gba->cpu->halted = 0;
	if (!(gba->memory.io[GBA_REG(IE)] & gba->memory.io[GBA_REG(IF)])) {
		return;
	}
