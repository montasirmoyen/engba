...
static void _verifyAwake(struct GBASIOLockstepCoordinator* coordinator) {
	...
	for (i = 0; i < coordinator->nAttached; ++i) {
		...
		struct GBASIOLockstepPlayer* player = static_cast<struct GBASIOLockstepPlayer*>(TableLookup(&coordinator->players, coordinator->attachedPlayers[i]));
...
static void _abortTransfer(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player) {
	...
	if (player->playerId != 0) {
		struct GBASIOLockstepPlayer* runner = static_cast<struct GBASIOLockstepPlayer*>(TableLookup(&coordinator->players, coordinator->attachedPlayers[0]));
		if (runner) {
...
static void GBASIOLockstepDriverDeinit(struct GBASIODriver* driver) {
	...
	player = static_cast<struct GBASIOLockstepPlayer*>(TableLookup(&coordinator->players, lockstep->lockstepId));
	if (player) {
...
static void GBASIOLockstepDriverReset(struct GBASIODriver* driver) {
	...
	if (!lockstep->lockstepId) {
		unsigned id;
		player = static_cast<struct GBASIOLockstepPlayer*>(calloc(1, sizeof(*player)));
...
static enum GBASIOMode _modeIntToEnum(unsigned mode) {
	const enum GBASIOMode modes[8] = {
		static_cast<enum GBASIOMode>(-1), GBA_SIO_MULTI, GBA_SIO_NORMAL_8, GBA_SIO_NORMAL_32, GBA_SIO_GPIO, GBA_SIO_UART, GBA_SIO_JOYBUS, static_cast<enum GBASIOMode>(-1)
	};
	return modes[mode & 7];
}
...
static bool GBASIOLockstepDriverLoadState(struct GBASIODriver* driver, const void* data, size_t size) {
	struct GBASIOLockstepDriver* lockstep = (struct GBASIOLockstepDriver*) driver;
	struct GBASIOLockstepCoordinator* coordinator = lockstep->coordinator;
	const struct GBASIOLockstepSerializedState* state;
	bool error = false;
	uint32_t ucheck;
	int32_t check;
	struct GBASIOLockstepPlayer* player;
	...
	unsigned i;
	int32_t when;
	struct GBASIOLockstepEvent** lastEvent;
	struct GBASIOLockstepEvent* event;
	const struct GBASIOLockstepSerializedEvent* stateEvent;
	GBASIOLockstepSerializedEventFlags eventFlags;
	uint32_t tempMode;
	...
	state = static_cast<const struct GBASIOLockstepSerializedState*>(data);
	...
	for (i = 0; i < GBASIOLockstepSerializedFlagsGetNumEvents(flags) && i < MAX_LOCKSTEP_EVENTS; ++i) {
		...
		*lastEvent = event;
		...
		event->type = static_cast<enum GBASIOLockstepEventType>(GBASIOLockstepSerializedEventFlagsGetType(eventFlags));
		switch (event->type) {
		case SIO_EV_ATTACH:
		case SIO_EV_DETACH:
		...
		case SIO_EV_MODE_SET:
			...
			LOAD_32LE(tempMode, 0, &stateEvent->mode);

			event->mode = static_cast<enum GBASIOMode>(tempMode);
			break;
...
static void GBASIOLockstepDriverSaveState(struct GBASIODriver* driver, void** stateOut, size_t* size) {
	struct GBASIOLockstepDriver* lockstep = (struct GBASIOLockstepDriver*) driver;
	struct GBASIOLockstepCoordinator* coordinator = lockstep->coordinator;
	struct GBASIOLockstepSerializedState* state = static_cast<struct GBASIOLockstepSerializedState*>(calloc(1, sizeof(*state)));
...
static bool GBASIOLockstepDriverStart(struct GBASIODriver* driver) {
	...
	struct GBASIOLockstepPlayer* player = static_cast<struct GBASIOLockstepPlayer*>(TableLookup(&coordinator->players, lockstep->lockstepId));
	if (player->playerId != 0) {
...
void GBASIOLockstepCoordinatorDetach(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepDriver* driver) {
	...
	MutexLock(&coordinator->mutex);
	player = static_cast<struct GBASIOLockstepPlayer*>(TableLookup(&coordinator->players, driver->lockstepId));
	if (player) {
		_removePlayer(coordinator, player);
	}
	MutexUnlock(&coordinator->mutex);
...
void _reconfigPlayers(struct GBASIOLockstepCoordinator* coordinator) {
	...
	if (players == 0) {
		...
		struct GBASIOLockstepPlayer* player = static_cast<struct GBASIOLockstepPlayer*>(TableIteratorGetValue(&coordinator->players, &iter));
		...
		for (i = 0; i < MAX_GBAS; ++i) {
			...
			for (j = 0; j <= i; ++j) {
				while (UIntListSize(&playerPreferences[j]) && seen < MAX_GBAS) {
					...
					struct GBASIOLockstepPlayer* player = static_cast<struct GBASIOLockstepPlayer*>(TableLookup(&coordinator->players, pid));
					if (!player) {
...
void _lockstepEvent(struct mTiming* timing, void* context, uint32_t cyclesLate) {
	struct GBASIOLockstepDriver* lockstep = static_cast<struct GBASIOLockstepDriver*>(context);
	struct GBASIOLockstepCoordinator* coordinator = lockstep->coordinator;
	MutexLock(&coordinator->mutex);
	player = static_cast<struct GBASIOLockstepPlayer*>(TableLookup(&coordinator->players, lockstep->lockstepId));
	...
	while (true) {
		...
		switch (event->type) {
		case SIO_EV_ATTACH:
			_setReady(coordinator, player, event->playerId, static_cast<enum GBASIOMode>(-1));
			if (player->playerId == 0) {
				struct GBASIO* sio = player->driver->d.p;
