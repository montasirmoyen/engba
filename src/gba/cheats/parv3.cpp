...
static bool _addPAR3Cond(struct GBACheatSet* cheats, uint32_t op1, uint32_t op2) {
	enum GBAActionReplay3Condition condition = static_cast<enum GBAActionReplay3Condition>(op1 & PAR3_COND);
...
bool GBACheatAddProActionReplayRaw(struct GBACheatSet* cheats, uint32_t op1, uint32_t op2) {
	...
	if (op1 >> 24 == 0xC4) {
		if (cheats->hook) {
			...
			cheats->hook = static_cast<struct GBACheatHook*>(malloc(sizeof(*cheats->hook)));
