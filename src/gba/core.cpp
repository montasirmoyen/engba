...
static const struct mCoreMemoryBlock _GBAMemoryBlocks[] = {
	{ static_cast<size_t>(-1), "mem", "All", "All", 0, 0x10000000, 0x10000000, mCORE_MEMORY_VIRTUAL },
...
static bool _GBACoreInit(struct mCore* core) {
	...
	struct ARMCore* cpu = static_cast<struct ARMCore*>(anonymousMemoryMap(sizeof(struct ARMCore)));
	struct GBA* gba = static_cast<struct GBA*>(anonymousMemoryMap(sizeof(struct GBA)));
	if (!cpu || !gba) {
		free(cpu);
		free(gba);
		return false;
...
static void _GBACoreDeinit(struct mCore* core) {
	ARMDeinit(static_cast<struct ARMCore*>(core->cpu));
	GBADestroy(static_cast<struct GBA*>(core->board));
...
static void _GBACoreSetSync(struct mCore* core, struct mCoreSync* sync) {
	struct GBA* gba = static_cast<struct GBA*>(core->board);
	gba->sync = sync;
}
...
#endif
	if (GBAIsMB(vf)) {
		return GBALoadMB(static_cast<struct GBA*>(core->board), vf);
	}
	gbacore->memoryBlockType = -2;
	return GBALoadROM(static_cast<struct GBA*>(core->board), vf);
}
...
static bool _GBACoreLoadBIOS(struct mCore* core, struct VFile* vf, int type) {
	...
	GBALoadBIOS(static_cast<struct GBA*>(core->board), vf);
	return true;
}

static bool _GBACoreLoadSave(struct mCore* core, struct VFile* vf) {
	return GBALoadSave(static_cast<struct GBA*>(core->board), vf);
}
...
static bool _GBACoreLoadPatch(struct mCore* core, struct VFile* vf) {
	...
	GBAApplyPatch(static_cast<struct GBA*>(core->board), &patch);
	return true;
}

static void _GBACoreUnloadROM(struct mCore* core) {
	struct GBACore* gbacore = (struct GBACore*) core;
	struct ARMCore* cpu = static_cast<struct ARMCore*>(core->cpu);
	if (gbacore->cheatDevice) {
	...
	GBAUnloadROM(static_cast<struct GBA*>(core->board));
}
...
static void _GBACoreChecksum(const struct mCore* core, void* data, enum mCoreChecksumType type) {
	...
	case mCHECKSUM_MD5:
		if (gba->romVf) {
			md5File(gba->romVf, static_cast<uint8_t*>(data));
		} else if (gba->mbVf) {
			md5File(gba->mbVf, static_cast<uint8_t*>(data));
		} else if (gba->memory.rom && gba->isPristine) {
			md5Buffer(gba->memory.rom, gba->pristineRomSize, static_cast<uint8_t*>(data));
		} else if (gba->memory.rom) {
			md5Buffer(gba->memory.rom, gba->memory.romSize, static_cast<uint8_t*>(data));
		} else {
			md5Buffer("", 0, static_cast<uint8_t*>(data));
		}
		break;
	case mCHECKSUM_SHA1:
		if (gba->romVf) {
			sha1File(gba->romVf, static_cast<uint8_t*>(data));
		} else if (gba->mbVf) {
			sha1File(gba->mbVf, static_cast<uint8_t*>(data));
		} else if (gba->memory.rom && gba->isPristine) {
			sha1Buffer(gba->memory.rom, gba->pristineRomSize, static_cast<uint8_t*>(data));
		} else if (gba->memory.rom) {
			sha1Buffer(gba->memory.rom, gba->memory.romSize, static_cast<uint8_t*>(data));
		} else {
			sha1Buffer("", 0, static_cast<uint8_t*>(data));
		}
		break;
	}
	return;
...
static void _GBACoreReset(struct mCore* core) {
	...
	ARMReset(static_cast<struct ARMCore*>(core->cpu));
	...
	if (forceSkip) {
		GBASkipBIOS(static_cast<struct GBA*>(core->board));
	}
...
static void _GBACoreRunFrame(struct mCore* core) {
	...
	while (gba->video.frameCounter == frameCounter && mTimingCurrentTime(&gba->timing) - startCycle < VIDEO_TOTAL_LENGTH + VIDEO_HORIZONTAL_LENGTH) {
		ARMRunLoop(static_cast<struct ARMCore*>(core->cpu));
	}
}
...
static void _GBACoreStep(struct mCore* core) {
	ARMRun(static_cast<struct ARMCore*>(core->cpu));
}
...
static bool _GBACoreLoadState(struct mCore* core, const void* state) {
	return GBADeserialize(static_cast<struct GBA*>(core->board), static_cast<const struct GBASerializedState*>(state));
}

static bool _GBACoreSaveState(struct mCore* core, void* state) {
	GBASerialize(static_cast<struct GBA*>(core->board), static_cast<struct GBASerializedState*>(state));
	return true;
}
...
static bool _GBACoreLoadExtraState(struct mCore* core, const struct mStateExtdata* extdata) {
	...
	bool ok = true;
	if (mStateExtdataGet(extdata, static_cast<enum mStateExtdataTag>(EXTDATA_SUBSYSTEM_START + GBA_SUBSYSTEM_VIDEO_RENDERER), &item)) {
	...
	if (gba->sio.driver && gba->sio.driver->driverId && gba->sio.driver->loadState &&
	    mStateExtdataGet(extdata, static_cast<enum mStateExtdataTag>(EXTDATA_SUBSYSTEM_START + GBA_SUBSYSTEM_SIO_DRIVER), &item)) {
		if ((uint32_t) item.size > sizeof(uint32_t)) {
			uint32_t type;
			LOAD_32(type, 0, item.data);
...
static bool _GBACoreSaveExtraState(struct mCore* core, struct mStateExtdata* extdata) {
	...
	if (size > 0 && buffer) {
		...
		mStateExtdataPut(extdata, static_cast<enum mStateExtdataTag>(EXTDATA_SUBSYSTEM_START + GBA_SUBSYSTEM_VIDEO_RENDERER), &item);
	}
	if (buffer) {
		free(buffer);
		buffer = NULL;
	...
	if (gba->sio.driver && gba->sio.driver->driverId && gba->sio.driver->saveState) {
		...
		if (size > 0 && buffer) {
			...
			STORE_32(type, 0, item.data);
			memcpy((void*) ((uintptr_t) item.data + sizeof(uint32_t)), buffer, size);
			mStateExtdataPut(extdata, static_cast<enum mStateExtdataTag>(EXTDATA_SUBSYSTEM_START + GBA_SUBSYSTEM_SIO_DRIVER), &item);
		}
		if (buffer) {
			free(buffer);
			buffer = NULL;
...
static uint32_t _GBACoreFrameCounter(const struct mCore* core) {
	const struct GBA* gba = static_cast<struct GBA*>(core->board);
...
static void _GBACoreGetGameInfo(const struct mCore* core, struct mGameInfo* info) {
	GBAGetGameInfo(static_cast<const struct GBA*>(core->board), info);
}
...
static void _GBACoreSetPeripheral(struct mCore* core, int type, void* periph) {
	...
	case mPERIPH_ROTATION:
		gba->rotationSource = static_cast<struct mRotationSource*>(periph);
		break;
	case mPERIPH_RUMBLE:
		gba->rumble = static_cast<struct mRumble*>(periph);
		break;
	case mPERIPH_GBA_LUMINANCE:
		gba->luminanceSource = static_cast<struct GBALuminanceSource*>(periph);
		break;
	case mPERIPH_GBA_LINK_PORT:
		GBASIOSetDriver(&gba->sio, static_cast<struct GBASIODriver*>(periph));
		break;
	default:
		return;
	}
...
static bool _GBACoreReadRegister(const struct mCore* core, const char* name, void* out) {
	struct ARMCore* cpu = static_cast<struct ARMCore*>(core->cpu);
	int32_t* value = static_cast<int32_t*>(out);
	switch (name[0]) {
	case 'r':
	case 'R':
		++name;
...
static bool _GBACoreWriteRegister(struct mCore* core, const char* name, const void* in) {
	...
	case 'C':
		if (strcmp(name, "cpsr") == 0) {
			...
			enum ExecutionMode mode = static_cast<enum ExecutionMode>(cpu->cpsr.t);
...
static void _GBACoreAttachDebugger(struct mCore* core, struct mDebugger* debugger) {
	...
	if (core->debugger) {
		GBADetachDebugger(static_cast<struct GBA*>(core->board));
	}
	GBAAttachDebugger(static_cast<struct GBA*>(core->board), static_cast<struct mDebugger*>(debugger));
...
static struct mCheatDevice* _GBACoreCheatDevice(struct mCore* core) {
	...
	if (!gbacore->cheatDevice) {
		...
		ARMHotplugAttach(static_cast<struct ARMCore*>(core->cpu), CPU_COMPONENT_CHEAT_DEVICE);
...
static void _GBACoreStartVideoLog(struct mCore* core, struct mVideoLogContext* context) {
	...
	struct GBASerializedState* state = static_cast<struct GBASerializedState*>(mVideoLogContextInitialState(context, NULL));
	state->id = 0;
	state->cpu.gprs[ARM_PC] = GBA_BASE_EWRAM;

	int channelId = mVideoLoggerAddChannel(context);
	struct mVideoLogger* logger = static_cast<struct mVideoLogger*>(malloc(sizeof(*logger)));
...
struct mCore* GBACoreCreate(void) {
	struct GBACore* gbacore = static_cast<struct GBACore*>(malloc(sizeof(*gbacore)));
...
static void _GBAVLPStartFrameCallback(void *context) {
	struct mCore* core = static_cast<struct mCore*>(context);
	struct GBACore* gbacore = (struct GBACore*) core;
	struct GBA* gba = static_cast<struct GBA*>(core->board);
...
static bool _GBAVLPLoadState(struct mCore* core, const void* state) {
	...
	gba->cpu->memory.store16(gba->cpu, GBA_BASE_IO | GBA_REG_IME, 0, NULL);
	gba->cpu->memory.store16(gba->cpu, GBA_BASE_IO | GBA_REG_IE, 0, NULL);
	GBAVideoDeserialize(&gba->video, static_cast<const struct GBASerializedState*>(state));
	GBAIODeserialize(gba, static_cast<const struct GBASerializedState*>(state));
