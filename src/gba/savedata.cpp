...
void GBASavedataInitFlash(struct GBASavedata* savedata) {
	...
	if (!savedata->vf) {
		...
		savedata->data = static_cast<uint8_t*>(anonymousMemoryMap(GBA_SIZE_FLASH1M));
	} else {
		end = savedata->vf->size(savedata->vf);
		if (end < flashSize) {
			savedata->vf->truncate(savedata->vf, flashSize);
		}
		savedata->data = static_cast<uint8_t*>(savedata->vf->map(savedata->vf, flashSize, savedata->mapMode));
	}
...
void GBASavedataInitEEPROM(struct GBASavedata* savedata) {
	...
	if (!savedata->vf) {
		...
		savedata->data = static_cast<uint8_t*>(anonymousMemoryMap(GBA_SIZE_EEPROM));
	} else {
		end = savedata->vf->size(savedata->vf);
		if (end < eepromSize) {
			savedata->vf->truncate(savedata->vf, eepromSize);
		}
		savedata->data = static_cast<uint8_t*>(savedata->vf->map(savedata->vf, eepromSize, savedata->mapMode));
	}
...
void GBASavedataInitSRAM(struct GBASavedata* savedata) {
	...
	if (!savedata->vf) {
		...
		savedata->data = static_cast<uint8_t*>(anonymousMemoryMap(GBA_SIZE_SRAM));
	} else {
		end = savedata->vf->size(savedata->vf);
		if (end < GBA_SIZE_SRAM) {
			savedata->vf->truncate(savedata->vf, GBA_SIZE_SRAM);
		}
		savedata->data = static_cast<uint8_t*>(savedata->vf->map(savedata->vf, GBA_SIZE_SRAM, savedata->mapMode));
	}
...
void GBASavedataInitSRAM512(struct GBASavedata* savedata) {
	...
	if (!savedata->vf) {
		...
		savedata->data = static_cast<uint8_t*>(anonymousMemoryMap(GBA_SIZE_SRAM512));
	} else {
		end = savedata->vf->size(savedata->vf);
		if (end < GBA_SIZE_SRAM512) {
			savedata->vf->truncate(savedata->vf, GBA_SIZE_SRAM512);
		}
		savedata->data = static_cast<uint8_t*>(savedata->vf->map(savedata->vf, GBA_SIZE_SRAM512, savedata->mapMode));
	}
	...
	case FLASH_STATE_CONTINUE:
		...
		if (address == FLASH_BASE_HI) {
			...
			case FLASH_COMMAND_NONE:
				...
				case FLASH_COMMAND_PROGRAM:
				case FLASH_COMMAND_SWITCH_BANK:
					savedata->command = static_cast<enum SavedataCommand>(value);
					break;
				default:
...
static void _ensureEeprom(struct GBASavedata* savedata, uint32_t size) {
	...
	if (savedata->vf->size(savedata->vf) < GBA_SIZE_EEPROM) {
		...
		savedata->data = static_cast<uint8_t*>(savedata->vf->map(savedata->vf, GBA_SIZE_EEPROM, savedata->mapMode));
...
void GBASavedataWriteEEPROM(struct GBASavedata* savedata, uint16_t value, uint32_t writeSize) {
	...
	default:
		savedata->command = static_cast<enum SavedataCommand>(value & 0x1);
		break;
	case EEPROM_COMMAND_PENDING:
		savedata->command = static_cast<enum SavedataCommand>(static_cast<int>(savedata->command) << 1);
		savedata->command = static_cast<enum SavedataCommand>(static_cast<int>(savedata->command) | (value & 0x1));
...
void GBASavedataRTCWrite(struct GBASavedata* savedata) {
	...
	savedata->vf->write(savedata->vf, &buffer, sizeof(buffer));
	if (!savedata->data) {
		savedata->data = static_cast<uint8_t*>(savedata->vf->map(savedata->vf, size, MAP_WRITE));
		if (savedata->type == GBA_SAVEDATA_FLASH1M) {
...
void GBASavedataDeserialize(struct GBASavedata* savedata, const struct GBASerializedState* state) {
	if (savedata->type != state->savedata.type) {
		...
		GBASavedataForceType(savedata, static_cast<enum GBASavedataType>(state->savedata.type));
	}
	savedata->command = static_cast<enum SavedataCommand>(state->savedata.command);
	GBASerializedSavedataFlags flags = state->savedata.flags;
	savedata->flashState = static_cast<enum FlashStateMachine>(GBASerializedSavedataFlagsGetFlashState(flags));
	...
	if (bank > 0 && savedata->type == GBA_SAVEDATA_FLASH512) {
		...
		if (savedata->vf) {
			...
			if (savedata->vf->size(savedata->vf) < GBA_SIZE_FLASH1M) {
				...
				savedata->data = static_cast<uint8_t*>(savedata->vf->map(savedata->vf, GBA_SIZE_FLASH1M, MAP_WRITE));
