...
void _drawScanlines(struct GBAVideoGLRenderer* glRenderer, int y) {
	...
	glBindFramebuffer(GL_FRAMEBUFFER, glRenderer->fbo[GBA_GL_FBO_BACKDROP]);
	glDrawBuffers(1, (GLenum[]) { GL_COLOR_ATTACHMENT0 });
	glClearBufferiv(GL_COLOR, 0, (GLint[]) { 32, static_cast<GLint>(glRenderer->target1Bd | (glRenderer->target2Bd * 2) | (glRenderer->blendEffect * 4)), static_cast<GLint>(glRenderer->blda), 0 });
	int i;
	for (i = 0; i < 4; ++i) {
		glScissor(i + 1, glRenderer->firstY, 1, y - glRenderer->firstY + 1);
		glClearBufferiv(GL_COLOR, 0, (GLint[]) { static_cast<GLint>(glRenderer->bg[i].priority),
		                                         static_cast<GLint>(glRenderer->bg[i].target1 | (glRenderer->bg[i].target2 << 1) | (glRenderer->blendEffect << 2)),
		                                         static_cast<GLint>(glRenderer->blda), 0 });
	}
...
void GBAVideoGLRendererGetPixels(struct GBAVideoRenderer* renderer, size_t* stride, const void** pixels) {
	...
	*stride = GBA_VIDEO_HORIZONTAL_PIXELS * glRenderer->scale;
	if (!glRenderer->temporaryBuffer) {
		glRenderer->temporaryBuffer = static_cast<uint32_t*>(anonymousMemoryMap(GBA_VIDEO_HORIZONTAL_PIXELS * GBA_VIDEO_VERTICAL_PIXELS * glRenderer->scale * glRenderer->scale * BYTES_PER_PIXEL));
	}
	glFinish();
	glBindFramebuffer(GL_FRAMEBUFFER, glRenderer->fbo[GBA_GL_FBO_OUTPUT]);
...
static void GBAVideoGLRendererWriteBLDCNT(struct GBAVideoGLRenderer* renderer, uint16_t value) {
	...
	renderer->blendEffect = static_cast<enum GBAVideoBlendEffect>(GBARegisterBLDCNTGetEffect(value));
...
void GBAVideoGLRendererDrawSprite(struct GBAVideoGLRenderer* renderer, struct GBAObj* sprite, int y, int spriteY) {
	...
	if (GBAObjAttributesAIsTransformed(sprite->a)) {
		...
		glUniformMatrix2fv(uniforms[GBA_GL_OBJ_TRANSFORM], 1, GL_FALSE, (GLfloat[]) { static_cast<GLfloat>(flipX), 0, 0, static_cast<GLfloat>(flipY) });
	}
