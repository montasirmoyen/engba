...
static void _breakWindow(struct GBAVideoSoftwareRenderer* softwareRenderer, struct GBAVideoSoftwareRenderer::WindowN* win);
static void _breakWindowInner(struct GBAVideoSoftwareRenderer* softwareRenderer, struct GBAVideoSoftwareRenderer::WindowN* win);
...
static void GBAVideoSoftwareRendererReset(struct GBAVideoRenderer* renderer) {
	...
	softwareRenderer->winN[0] = (struct GBAVideoSoftwareRenderer::WindowN) { .control = { .priority = 0 } };
	softwareRenderer->winN[1] = (struct GBAVideoSoftwareRenderer::WindowN) { .control = { .priority = 1 } };
...
static void _breakWindow(struct GBAVideoSoftwareRenderer* softwareRenderer, struct GBAVideoSoftwareRenderer::WindowN* win) {
	if (win->h.end > GBA_VIDEO_HORIZONTAL_PIXELS || win->h.end < win->h.start) {
		struct GBAVideoSoftwareRenderer::WindowN splits[2] = { *win, *win };
		splits[0].h.start = 0;
...
static void GBAVideoSoftwareRendererFinishFrame(struct GBAVideoRenderer* renderer) {
	...
	int i;
	for (i = 0; i < 2; ++i) {
		struct GBAVideoSoftwareRenderer::WindowN* win = &softwareRenderer->winN[i];
...
static void GBAVideoSoftwareRendererPutPixels(struct GBAVideoRenderer* renderer, size_t stride, const void* pixels) {
	...
	const mColor* colorPixels = static_cast<const mColor*>(pixels);
	unsigned i;
