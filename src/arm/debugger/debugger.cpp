...
static bool ARMDebuggerUpdateStackTraceInternal(struct mDebuggerPlatform* d, uint32_t pc) {
	...
	enum RegisterBank currentStack = ARMSelectBank(static_cast<enum PrivilegeMode>(cpu->cpsr.priv));
	if (frame && frame->frameBaseAddress < (uint32_t) cpu->gprs[ARM_SP] && currentStack == ARMSelectBank(static_cast<enum PrivilegeMode>(FRAME_PRIV(frame)))) {
		...
		} while (frame && frame->frameBaseAddress < (uint32_t) cpu->gprs[ARM_SP] && currentStack == ARMSelectBank(static_cast<enum PrivilegeMode>(FRAME_PRIV(frame))));
		if (shouldBreak) {
	...
	if (_ARMModeHasSPSR(static_cast<PrivilegeMode>(cpu->cpsr.priv))) {
		...
		if (ivtBase <= pc && pc < ivtBase + 0x20 && !(irqFrame && _ARMModeHasSPSR(static_cast<PrivilegeMode>(((struct ARMRegisterFile*) irqFrame->regs)->cpsr.priv)))) {
	...
	if (interrupt && !isCall) {
		...
		if (isCall) {
			...
			bool isExceptionReturn = _ARMModeHasSPSR(static_cast<PrivilegeMode>(cpu->cpsr.priv)) && info.affectsCPSR && info.op1.reg == ARM_PC;
			bool isMovPcLr = (info.operandFormat & ARM_OPERAND_REGISTER_2) && info.op1.reg == ARM_PC && info.op2.reg == ARM_LR;
			bool isBranch = ARMInstructionIsBranch(static_cast<enum ARMMnemonic>(info.mnemonic));
	...
	if (interrupt || isCall) {
		...
		if (frame && currentStack == ARMSelectBank(static_cast<enum PrivilegeMode>(FRAME_PRIV(frame)))) {
			mStackTracePop(stack);
		}
...
static void ARMDebuggerCheckBreakpoints(struct mDebuggerPlatform* d) {
	...
	for (i = 0; i < ARMDebugBreakpointListSize(&debugger->breakpoints); ++i) {
		...
		struct mDebuggerEntryInfo info = {
			...
			.target = static_cast<struct mDebuggerModule*>(TableLookup(&d->p->pointOwner, breakpoint->d.id))
		};
...
void ARMDebuggerInit(void* cpu, struct mDebuggerPlatform* platform) {
	...
	debugger->cpu = static_cast<struct ARMCore*>(cpu);
...
static void ARMDebuggerFrameFormatRegisters(struct mStackFrame* frame, char* out, size_t* length) {
	ARMDebuggerFormatRegisters(static_cast<struct ARMRegisterFile*>(frame->regs), out, length);
}
