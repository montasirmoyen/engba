...
#define ARM_ADDITION_S(M, N, D) \
	if (rd == ARM_PC && _ARMModeHasSPSR(static_cast<PrivilegeMode>(cpu->cpsr.priv))) { \
		cpu->cpsr = cpu->spsr; \
		_ARMReadCPSR(cpu); \
	} else { \
...
#define ARM_MS_POST_load \
	...
	} else if (_ARMModeHasSPSR(static_cast<PrivilegeMode>(cpu->cpsr.priv))) { \
		cpu->cpsr = cpu->spsr; \
		_ARMReadCPSR(cpu); \
	}
...
DEFINE_INSTRUCTION_ARM(BX,
	int rm = opcode & 0x0000000F;
	_ARMSetMode(cpu, static_cast<ExecutionMode>(cpu->gprs[rm] & 0x00000001));
