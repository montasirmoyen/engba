...
static void _cleanTree(void* value) {
	struct TextCodecNode* node = static_cast<struct TextCodecNode*>(value);
	if (node->leaf) {
		free(node->leaf);
	}
...
static struct TextCodecNode* _createNode(void) {
	struct TextCodecNode* node = static_cast<struct TextCodecNode*>(malloc(sizeof(*node)));
	node->leaf = NULL;
	node->leafLength = 0;
...
static void _insertLeaf(struct TextCodecNode* node, uint8_t* word, size_t wordLength, uint8_t* output, size_t outputLength) {
	if (!wordLength) {
		...
		node->leaf = static_cast<uint8_t*>(malloc(outputLength));
		memcpy(node->leaf, output, outputLength);
		return;
	}
	struct TextCodecNode* subnode = static_cast<struct TextCodecNode*>(TableLookup(&node->children, word[0]));
	if (!subnode) {
		subnode = _createNode();
...
ssize_t TextCodecAdvance(struct TextCodecIterator* iter, uint8_t byte, uint8_t* output, size_t outputLength) {
	struct TextCodecNode* node = static_cast<struct TextCodecNode*>(TableLookup(&iter->current->children, byte));
	if (!node) {
