...
static struct TableList* _resizeAsNeeded(struct Table* table, struct TableList* list, uint32_t key) {
	...
	if (list->nEntries + 1 == list->listSize) {
		list->listSize *= 2;
		list->list = static_cast<struct TableTuple*>(realloc(list->list, list->listSize * sizeof(struct TableTuple)));
	}
	return list;
}
...
void TableInit(struct Table* table, size_t initialSize, void (*deinitializer)(void*)) {
	...
	table->table = static_cast<struct TableList*>(calloc(table->tableSize, sizeof(struct TableList)));
	table->size = 0;
	...
	for (i = 0; i < table->tableSize; ++i) {
		...
		table->table[i].list = static_cast<struct TableTuple*>(calloc(LIST_INITIAL_SIZE, sizeof(struct TableTuple)));
	}
}
...
void TableClear(struct Table* table) {
	...
	for (i = 0; i < table->tableSize; ++i) {
		...
		list->nEntries = 0;
		list->list = static_cast<struct TableTuple*>(calloc(LIST_INITIAL_SIZE, sizeof(struct TableTuple)));
	}
}
...
void HashTableInsertBinary(struct Table* table, const void* key, size_t keylen, void* value) {
	...
	list = _resizeAsNeeded(table, list, hash);
	list->list[list->nEntries].key = hash;
	list->list[list->nEntries].stringKey = static_cast<char*>(malloc(keylen));
...
void HashTableInsertCustom(struct Table* table, void* key, void* value) {
	...
	list = _resizeAsNeeded(table, list, hash);
	list->list[list->nEntries].key = hash;
	list->list[list->nEntries].stringKey = static_cast<char*>(table->fn.ref(key));
...
void HashTableInsertBinaryMoveKey(struct Table* table, void* key, size_t keylen, void* value) {
	...
	list = _resizeAsNeeded(table, list, hash);
	list->list[list->nEntries].key = hash;
	list->list[list->nEntries].stringKey = static_cast<char*>(const_cast<void*>(key));
	list->list[list->nEntries].keylen = keylen;
	list->list[list->nEntries].value = value;
	++list->nEntries;
	++table->size;
...
static bool HashTableMemcmp(const char* key, const void* value, const void* user) {
	UNUSED(key);
	const struct HashTableSearchParam* ref = static_cast<const struct HashTableSearchParam*>(user);
...
static bool HashTableStrcmp(const char* key, const void* value, const void* user) {
	UNUSED(key);
	return strcmp(static_cast<const char*>(value), static_cast<const char*>(user)) == 0;
}
