...
static void _pngWrite(png_structp png, png_bytep buffer, png_size_t size) {
	struct VFile* vf = static_cast<struct VFile*>(png_get_io_ptr(png));
...
bool PNGWritePixels(png_structp png, unsigned width, unsigned height, unsigned stride, const void* pixels, enum mColorFormat fmt) {
	...
	png_bytep row = static_cast<png_bytep>(malloc(sizeof(png_byte) * width * depth));
	if (!row) {
		return false;
	}
	const png_byte* pixelData = static_cast<const png_byte*>(pixels);
	if (setjmp(png_jmpbuf(png))) {
		free(row);
		return false;
	}
...
bool PNGWriteCustomChunk(png_structp png, const char* name, size_t size, void* data) {
	...
	png_write_chunk(png, reinterpret_cast<png_bytep>(const_cast<char*>(realName)), static_cast<png_const_bytep>(data), size);
	return true;
}
...
bool PNGReadPixels(png_structp png, png_infop info, void* pixels, unsigned width, unsigned height, unsigned stride) {
	...
	unsigned i;
	png_bytep row = static_cast<png_bytep>(malloc(png_get_rowbytes(png, info)));
