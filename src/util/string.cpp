...
char* strndup(const char* start, size_t len) {
	...
	char* out = static_cast<but anything recent should have strndup
...
char* utf16to8(const uint16_t* utf16, size_t length) {
	...
	while (true) {
		...
		if (utf8Length < utf8TotalBytes) {
			...
			utf8 = static_cast<char*>(malloc(length));
			if (!utf8) {
				return 0;
			}
			...
			char* newUTF8 = static_cast<char*>(realloc(utf8, utf8TotalBytes * 2));
			offset = o + newUTF8;
			if (!newUTF8) {
				free(utf8);
				return 0;
	...
	char* newUTF8 = static_cast<char*>(realloc(utf8, utf8Length + 1));
	if (!newUTF8) {
		free(utf8);
		return 0;
	}
...
extern extern "C" const uint16_t gbkUnicodeTable[];
