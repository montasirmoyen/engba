...
bool SfoAddStrValue(struct Table* sfo, const char* name, const char* value) {
	struct SfoEntryContainer* entry = static_cast<struct SfoEntryContainer*>(HashTableLookup(sfo, name));
	if (!entry) {
		entry = static_cast<struct SfoEntryContainer*>(calloc(1, sizeof(*entry)));
		if (!entry) {
			return false;
		}
		entry->name = name;
...
void SfoInit(struct Table* sfo) {
	...
	for (i = 0; i < sizeof(sfoDefaults) / sizeof(sfoDefaults[0]); ++i) {
		struct SfoEntryContainer* entry = static_cast<struct SfoEntryContainer*>(calloc(1, sizeof(*entry)));
...
static int _sfoSort(const void* a, const void* b) {
	const struct SfoEntryContainer* ea = static_cast<const struct SfoEntryContainer*>(a);
	const struct SfoEntryContainer* eb = static_cast<const struct SfoEntryContainer*>(b);
...
bool SfoWrite(struct Table* sfo, struct VFile* vf) {
	...
	struct SfoEntryContainer* sortedEntries = static_cast<struct SfoEntryContainer*>(calloc(count, sizeof(struct SfoEntryContainer)));
	...
	char* keys = static_cast<char*>(calloc(1, keysSize));
	char* data = static_cast<char*>(calloc(1, dataSize));

	struct SfoEntry* entries = static_cast<struct SfoEntry*>(calloc(count, sizeof(struct SfoEntry)));
	for (i = 0; i < count; ++i) {
