...
struct VDir* VDirOpen7z(const char* path, int flags) {
	...
	struct VDir7z* vd = static_cast<struct VDir7z*>(malloc(sizeof(struct VDir7z)));
	...
	vd->lookStream.buf = static_cast<Byte*>(malloc(BUFFER_SIZE));
...
struct VFile* _vd7zOpenFile(struct VDir* vd, const char* path, int mode) {
	...
	for (i = 0; i < vd7z->db.NumFiles; ++i) {
		...
		UInt16* name = static_cast<UInt16*>(malloc(nameLength));
	...
	struct VFile7z* vf = static_cast<struct VFile7z*>(malloc(sizeof(struct VFile7z)));
	vf->vd = vd7z;
