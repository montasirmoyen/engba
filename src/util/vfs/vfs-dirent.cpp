...
struct VDir* VDirOpen(const char* path) {
	...
	struct VDirDE* vd = static_cast<struct VDirDE*>(malloc(sizeof(struct VDirDE)));
	if (!vd) {
		closedir(de);
		return 0;
	}
...
struct VFile* _vdOpenFile(struct VDir* vd, const char* path, int mode) {
	...
	char* combined = static_cast<char*>(malloc(sizeof(char) * (strlen(path) + strlen(dir) + 2)));
...
static enum VFSType _vdeType(struct VDirEntry* vde) {
	...
	char* combined = static_cast<char*>(malloc(sizeof(char) * (strlen(vdede->ent->d_name) + strlen(dir) + 2)));
