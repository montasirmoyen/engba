...
static uLong _vfmzRead(voidpf opaque, voidpf stream, void* buf, uLong size) {
	UNUSED(opaque);
	struct VFile* vf = static_cast<struct VFile*>(stream);
...
#endif
	struct VDirZip* vd = static_cast<struct VDirZip*>(malloc(sizeof(struct VDirZip)));
...
struct VFile* _vdzOpenFile(struct VDir* vd, const char* path, int mode) {
	...
	struct VFileZip* vfz = static_cast<struct VFileZip*>(calloc(1, sizeof(struct VFileZip)));
	vfz->uz = vdz->uz;
	vfz->z = vdz->z;
	vfz->buffer = 0;
	vfz->bufferSize = 0;
