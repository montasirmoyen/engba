...
void mInterpolatorSincInit(struct mInterpolatorSinc* interp, unsigned resolution, unsigned width) {
	...
	interp->sincLut = static_cast<double*>(calloc(samples + 1, sizeof(double)));
	interp->windowLut = static_cast<double*>(calloc(samples + 1, sizeof(double)));
...
void mInterpolatorCosineInit(struct mInterpolatorCosine* interp, unsigned resolution) {
	...
	interp->lut = static_cast<double*>(calloc(resolution + 1, sizeof(double)));
