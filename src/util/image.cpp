...
struct mImage* mImageCreateWithStride(unsigned width, unsigned height, unsigned stride, enum mColorFormat format) {
	...
	struct mImage* image = static_cast<struct mImage*>(calloc(1, sizeof(struct mImage)));
	if (!image) {
		return NULL;
	}
	image->width = width;
	...
	if (format == mCOLOR_PAL8) {
		image->palette = static_cast<uint32_t*>(malloc(1024));
		if (!image->palette) {
			free(image->data);
			free(image);
			return NULL;
...
static struct mImage* mImageLoadPNG(struct VFile* vf) {
	...
	struct mImage* image = static_cast<struct mImage*>(calloc(1, sizeof(*image)));
	if (!image) {
		PNGReadClose(png, info, end);
		return NULL;
	}
...
struct mImage* mImageConvertToFormat(const struct mImage* image, enum mColorFormat format) {
	...
	struct mImage* newImage = static_cast<struct mImage*>(calloc(1, sizeof(*newImage)));
...
#define COMPOSITE_BOUNDS_INIT(SOURCE, DEST) \
	struct mRectangle dstRect = { \
		.x = 0, \
		.y = 0, \
		.width = static_cast<int>((DEST)->width), \
		.height = static_cast<int>((DEST)->height) \
	}; \
...
#define FILL_BOUNDS_INIT(X, Y, W, H) \
	struct mRectangle dstRect = { \
		.x = 0, \
		.y = 0, \
		.width = static_cast<int>(painter->backing->width), \
		.height = static_cast<int>(painter->backing->height) \
	}; \
	struct mRectangle srcRect = { \
		.x = (X), \
		.y = (Y), \
