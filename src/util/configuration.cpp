...
static void _tableDeinit(void* table) {
	TableDeinit(static_cast<struct Table*>(table));
	free(table);
}
...
static int _iniRead(void* configuration, const char* section, const char* key, const char* value) {
	...
	ConfigurationSetValue(static_cast<struct Configuration*>(configuration), section, key, value);
	return 1;
}

static void _keyHandler(const char* key, void* value, void* user) {
	char line[256];
	struct VFile* vf = static_cast<struct VFile*>(user);
...
static void _sectionHandler(const char* key, void* section, void* user) {
	...
	vf->write(vf, line, len);
	HashTableEnumerate(static_cast<const struct Table*>(section), _keyHandler, user);
	vf->write(vf, "\n", 1);
}

static void _sectionEnumHandler(const char* key, void* section, void* user) {
	struct ConfigurationSectionHandlerData* data = static_cast<struct ConfigurationSectionHandlerData*>(user);
	UNUSED(section);
	data->handler(key, data->data);
}

static void _enumHandler(const char* key, void* value, void* user) {
	struct ConfigurationHandlerData* data = static_cast<struct ConfigurationHandlerData*>(user);
	data->handler(key, static_cast<const char*>(value), data->data);
}
...
void ConfigurationSetValue(struct Configuration* configuration, const char* section, const char* key, const char* value) {
	struct Table* currentSection = static_cast<struct Table*>(&configuration->root);
	if (section) {
		currentSection = static_cast<struct Table*>(HashTableLookup(&configuration->sections, section));
		if (!currentSection) {
			if (value) {
				currentSection = static_cast<struct Table*>(malloc(sizeof(*currentSection)));
...
bool ConfigurationHasSection(const struct Configuration* configuration, const char* section) {
	return static_cast<const char*>(HashTableLookup(&configuration->sections, section));
}
...
const char* ConfigurationGetValue(const struct Configuration* configuration, const char* section, const char* key) {
	const struct Table* currentSection = static_cast<const struct Table*>(&configuration->root);
	if (section) {
		currentSection = static_cast<struct Table*>(HashTableLookup(&configuration->sections, section));
		if (!currentSection) {
			return 0;
	...
	return static_cast<const char*>(HashTableLookup(currentSection, key));
}
